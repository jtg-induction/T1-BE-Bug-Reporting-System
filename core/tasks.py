...
def send_ticket_assignment_email(
    ...
    ticket_url = (
        f"{FRONTEND_BASE_URL}/project/{project_id}/tickets/{ticket_id}"
    )
...
def notify_ticket_subscribers(self, ticket_id, new_status):
    ...
    ticket_url = (
        f"{FRONTEND_BASE_URL}/project/{ticket.project.id}/tickets/{ticket.id}"
    )
    ...
    email_message = EmailMessage(
        subject=subject, body=html_content, bcc=emails
    )
...
def notify_reporter_resolved(
    self, reporter_email, ticket_title, ticket_id, project_id
):
...
def send_deadline_reminder(self, ticket_id, is_two_hour=False):
    ...
    ticket_url = (
        f"{FRONTEND_BASE_URL}/projects/{ticket.project.id}/tickets/{ticket.id}"
    )

    if ticket.status == Ticket.Status.RESOLVED:
        status_msg = "This ticket is marked as RESOLVED. Please verify if it can be closed."
    else:
