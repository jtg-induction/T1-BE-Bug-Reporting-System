...
from tickets.serializers import TicketListSerializer,TicketReadSerializer,TicketWriteSerializer
from core.tasks import send_ticket_assignment_email,notify_ticket_subscribers,notify_reporter_resolved,send_deadline_reminder
...
class UserTicketViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        ...
        Q(assignee=user) | 
        Q(reporter=user) | 
        Q(subscribers__user=user, subscribers__status=TicketSubscriber.Status.SUBSCRIBED)
...
class ProjectTicketViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if project.status != 1:
            raise ValidationError({"project": "Cannot add tickets to an inactive project."})
        ...
        if not is_admin:
            raise PermissionDenied("You do not have permission to create tickets in this project. Admin role required.")
        ...
        try:
            with transaction.atomic():
                ...
                if project.jira_url and user.jira_access_token:
                            ...
                            "content": [
                                {
                                    ...
                                    "content": [
                                        {
                                            "type": "text", 
                                            "text": ticket.description or "No description provided."
                                        }
                                    ]
                                }
                            ]
                    ...
                    if response.status_code == 201:
                        ...
                        raise ValueError(f"Jira rejected the ticket. Errors: {field_errors or error_msgs}")
            ...
            return Response(
                {"error": "Network error while contacting Jira. Ticket creation cancelled.", "details": str(e)},
                status=status.HTTP_503_SERVICE_UNAVAILABLE
            )
        except Exception as e:
            return Response(
                {"error": "An internal server error occurred. Ticket creation cancelled.", "details": str(e)},
    ...
    def unsubscribe(self, request, project_id=None, ticket_id=None):
        ...
        subscription, created = TicketSubscriber.objects.update_or_create(
            ...
            ticket=ticket,
            defaults={
                'status': TicketSubscriber.Status.UNSUBSCRIBED
            }
        )
    ...
    def update(self, request, *args, **kwargs):
        ...
        if project.status != 1:
            raise ValidationError({"project": "Cannot update tickets in an inactive project."})

        is_reporter = (ticket.reporter == user)
        is_assignee = (ticket.assignee == user)
        is_admin = ProjectMember.objects.filter(
            project=project, member=user, 
            role=ProjectMember.Role.ADMIN, status=ProjectMember.Status.ACTIVE
        ).exists()
        ...
        if not is_admin and not is_reporter:
            for field in data.keys():
                if field != 'status':
                    raise PermissionDenied(f"Assignees can only update the status. Cannot edit '{field}'.")
        ...
        jira_auth = HTTPBasicAuth(user.email, user.jira_access_token) if user.jira_access_token else None
        jira_headers = {"Accept": "application/json", "Content-Type": "application/json"}

        try:
            with transaction.atomic():
                serializer = self.get_serializer(ticket, data=general_data, partial=True)
                serializer.is_valid(raise_exception=True)
                ...
                if project.jira_url and jira_auth and updated_ticket.jira_id:
                    fields = {}
                    if 'title' in data: fields["summary"] = updated_ticket.title
                    if 'description' in data:
                        fields["description"] = {
                            "type": "doc", "version": 1, 
                            "content": [{"type": "paragraph", "content": [{"type": "text", "text": updated_ticket.description or "No description."}]}]
                        }
                    if 'severity' in data: fields["priority"] = {"name": updated_ticket.get_severity_display()}

                    if 'assignee' in data:
                        if updated_ticket.assignee and hasattr(updated_ticket.assignee, 'jira_account_id'):
                            fields["assignee"] = {"id": updated_ticket.assignee.jira_account_id}
                        else:
                            fields["assignee"] = None

                    if 'deadline' in data: fields["duedate"] = updated_ticket.deadline.strftime('%Y-%m-%d')

                    if fields:
                        url = f"{project.jira_url.rstrip('/')}/rest/api/3/issue/{updated_ticket.jira_id}"
                        res = requests.put(url, json={"fields": fields}, auth=jira_auth, headers=jira_headers, timeout=10)
                        if res.status_code != 204:
                            raise ValueError(f"Jira rejected field updates: {res.text}")

        except Exception as e:
            return Response({"error": f"General update failed: {str(e)}"}, status=status.HTTP_400_BAD_REQUEST)\
            ...
            try:
                with transaction.atomic():
                    ...
                    if project.jira_url and jira_auth and ticket.jira_id:
                        ...
                        t_res = requests.get(f"{jira_url}/rest/api/3/issue/{ticket.jira_id}/transitions", auth=jira_auth, headers=jira_headers, timeout=10)
                        if t_res.status_code == 200:
                            transitions = t_res.json().get('transitions', [])

                            status_map = {"open": "to do", "in progress": "in progress", "resolved": "in progress", "closed": "done"}
                            jira_target = status_map.get(ticket.get_status_display().lower())

                            trans_id = next((t['id'] for t in transitions if t['to']['name'].lower() == jira_target), None)
                            
                            if trans_id:
                                m_res = requests.post(f"{jira_url}/rest/api/3/issue/{ticket.jira_id}/transitions", json={"transition": {"id": trans_id}}, auth=jira_auth, headers=jira_headers, timeout=10)
                                if m_res.status_code != 204:
                                    raise ValueError(f"Jira status update rejected: {m_res.text}")
                            else:
                                raise ValueError(f"No valid Jira transition for status: {jira_target}")

                    notify_ticket_subscribers.delay(ticket_id=str(ticket.id), new_status=ticket.get_status_display())
                    if ticket.status == Ticket.Status.RESOLVED:
                        notify_reporter_resolved.delay(reporter_email=ticket.reporter.email, ticket_title=ticket.title, ticket_id=str(ticket.id), project_id=str(project.id))
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if project.status != 1:
            raise ValidationError({"project": "Cannot delete tickets in an inactive project."})
        ...
        try:
            with transaction.atomic():
                ...
                if jira_url and jira_token and jira_id:
                    ...
                    if response.status_code != 204:
                        ...
                        error_msgs = response_data.get("errorMessages", [])
                        field_errors = response_data.get("errors", {})
                        error_detail = field_errors or error_msgs or response_data.get("error")
                        raise ValueError(f"Jira rejected the deletion. Errors: {error_detail}")
            ...
            return Response(
                {"error": "Network error while contacting Jira. Ticket deletion cancelled.", "details": str(e)},
                status=status.HTTP_503_SERVICE_UNAVAILABLE
            )
        except Exception as e:
            return Response(
                {"error": "An internal server error occurred. Ticket deletion cancelled.", "details": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def movable_projects(self, request, project_id=None, ticket_id=None):
        ...
        compatible_projects = Project.objects.filter(
            id__in=admin_project_ids,
            status=1, 
            jira_url=current_project.jira_url
